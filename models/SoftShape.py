...
class SoftShapeModel(nn.Module):
    ...
    def forward(self, x,_,__,_a, num_epoch_i=100, warm_up_epoch=50):
        x=x.transpose(1,2)
...
def Model(config):
    return SoftShapeModel(seq_len=config.seq_len,emb_dim=config.d_model,depth=config.e_layers,num_channels=config.enc_in,shape_size=8,sparse_rate=0.5,num_classes=config.num_class)