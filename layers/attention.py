...
# class ScaledDotProductAttention(nn.Module):
#     r"""Scaled Dot-Product Attention module with causal attention support
#     (Attention is all you need by Vaswani et al., 2017) with optional residual attention from previous layer
...
#     def __init__(self, d_model, n_heads, attn_dropout=0., res_attention=False, lsa=False, causal=True):
#         super().__init__()
#         self.attn_dropout = nn.Dropout(attn_dropout)
#         self.res_attention = res_attention
#         self.causal = causal  # 新增因果注意力标志
#         head_dim = d_model // n_heads
#         self.scale = nn.Parameter(torch.tensor(head_dim ** -0.5), requires_grad=lsa)
#         self.lsa = lsa
...
#         # Causal attention mask (applied before any other masks)
#         if self.causal:
#             # 创建因果掩码，确保每个位置只能关注到它之前的位置
#             q_len, k_len = q.size(2), k.size(3)
#             causal_mask = torch.triu(torch.ones((q_len, k_len), dtype=torch.bool, device=q.device), diagonal=1)
#             attn_scores.masked_fill_(causal_mask.unsqueeze(0).unsqueeze(1), float('-inf'))
