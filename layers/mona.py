...
class InceptionBlock(nn.Module):
    def __init__(
        ...
        self.residual = nn.Sequential(
            nn.Conv1d(
                in_channels=in_channels,
                out_channels=self.total_module_out,
                kernel_size=1,
                padding="same",
                padding_mode=padding_mode,
            ),
            nn.BatchNorm1d(num_features=self.total_module_out),
        )
        self.channel_align = nn.Conv1d(
            self.total_module_out,
            self.out_channels,
            kernel_size=1,
            padding="same"
        )
        self.liner=nn.Linear(self.total_module_out,self.out_channels)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x_main = self.inception_modules(x)
        x_res = self.residual(x)
        x=F.selu(x_main + x_res)
        # x_main = self.channel_align(x_main)
        # x_res = self.channel_align(x_res)
        return self.liner(x)
