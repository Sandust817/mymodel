...
def set_seed(fix_seed):
    ...
    # torch.random.manual_seed(fix_seed)  # 补充torch.random的种子
    
    # # 3. GPU相关随机种子与设置
    # torch.cuda.manual_seed(fix_seed)
    # torch.cuda.manual_seed_all(fix_seed)  # 多GPU时同步所有设备种子
    
    # # 禁用cuDNN的自动算法选择（避免卷积算法随机性）
    # # torch.backends.cudnn.benchmark = False    
    # # torch.backends.cudnn.deterministic = True 
    # # torch.use_deterministic_algorithms(True, warn_only=True)
    
    # # 4. 数据加载子进程种子（解决DataLoader worker随机性）
    def seed_worker(worker_id):
        worker_seed = fix_seed + worker_id  # 每个worker分配独立种子
        random.seed(worker_seed)
        np.random.seed(worker_seed)
        torch.manual_seed(worker_seed)
    
    # 创建DataLoader时需传入的生成器（关联worker种子）
    g = torch.Generator()
    g.manual_seed(fix_seed)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--seed', type=int, default=998244353, help="Randomization seed")
